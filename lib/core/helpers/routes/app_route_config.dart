...
import 'package:payments_application/core/helpers/internet_connectivity/view/connectivity_page.dart';
...
import '../internet_connectivity/controller/internet_connectivity_provider.dart';
import '../session/controller/session_controller.dart';
...
class AppRoutes {
  ...
  AppRoutes({required BuildContext context}) {
    final sessionManager = context.read<SessionManager>();
    final connectivityManager = context.read<ConnectivityProvider>();
    appRouter = GoRouter(
      ...
      redirect: (context, state) {
        if (state.matchedLocation == RoutesPath.splash) {
          final String curSession =
              state.uri.queryParameters['session']?.trim() ?? '';
          final bool sessionActive = sessionManager.sessionActive;
          final bool isConnected = connectivityManager.isConnected;

          if (!isConnected) {
            return RoutesPath.connectivity_page;
          }

          if (curSession.isEmpty) {
            return RoutesPath.error_page;
          }
          ...
          if (sessionActive) {
            ...
            Future.microtask(() {
              Provider.of<HomeProvider>(context, listen: false)
                  .sessionApi(context: context, curSession: curSession)
                  .then((_) {
                return RoutesPath.home;
              });
            });
