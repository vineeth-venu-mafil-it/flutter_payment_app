...
import '../model/pay_imps_res_det.dart';
import '../model/pay_neft_cus_det.dart';
import '../model/pay_report_model.dart';
import '../model/pay_rpt_neft_sft_det.dart';
...
class PayReportController extends ChangeNotifier {
  ...
  Future<void> payReportAccess({required BuildContext context}) async {
    try {
      final response = await _api.payReportAccess();
      ...
      if (response != null && response['status'] == 200) {
        ...
        if (result != null && result is List && result.isNotEmpty) {
          final item = (response['data']['result'] as List)
              .map((item) => PayReportModel.fromJson(item))
              .toList();
          oglReport = item;
          notifyListeners();
        } else {
  ...
  ///get doc id
  List<PayTransDet> transDet = [];
  List<PayNeftCusDet> neftCusDet = [];
    ...
    try {
      ...
      if (response != null && response['status'] == 200) {
        ...
        final items = (response['data']['response'][0]['trans_detail'] as List)
            .map((item) => PayTransDet.fromJson(item))
            .toList();
        transDet = items;
        ...
        final items1 =
            (response['data']['response'][0]['neft_cust_details'] as List)
                .map((item) => PayNeftCusDet.fromJson(item))
                .toList();
        neftCusDet = items1;
  ...
  ///get cor id
  List<PayReImpSendDet> impsSendDtl = [];
  ...
  Future<void> exportToExcel(
      List<PayReportModel> reportData, BuildContext context) async {
    try {
      ...
      String fileName = 'pay_report$formattedDate.xlsx';
  ...
  Future<void> exportToExcelImpsSntDt(
      List<PayReImpSendDet> reportData, BuildContext context) async {
    try {
      var excel = Excel.createExcel();
      Sheet sheetObject = excel['Sheet1'];
  ...
  Future<void> exportToExcelImpsResDt(
      List<PayRptImpsResDt> reportData, BuildContext context) async {
    try {
      var excel = Excel.createExcel();
      Sheet sheetObject = excel['Sheet1'];
  ...
  Future<void> exportToExcelneftSftDet(
      List<PayRptneftSftDet> reportData, BuildContext context) async {
    try {
      var excel = Excel.createExcel();
      Sheet sheetObject = excel['Sheet1'];
  ...
  Future<void> exportToExcelSbiTransDet(
      List<PayRptSbiTransDet> reportData, BuildContext context) async {
    try {
      var excel = Excel.createExcel();
      Sheet sheetObject = excel['Sheet1'];
  ...
  Future<void> savePdfPayReport(
    ...
    String formattedDate = DateFormat('MMM d, yyyy').format(DateTime.now());
    String fileName = 'payment_report_$formattedDate.pdf';
